<template>
  <div
    role="status"
    class="flex items-center justify-center h-[70vh] xl:h-[80vh] transition-opacity duration-300"
    :class="{ visible: isLoading, hidden: !isLoading }"
  >
    <svg
      aria-hidden="true"
    ...
    <div class="flex items-center w-full relative h-full ml-20 sm:ml-0">
      ...
      <div class="w-full ml-5">
        <div
          ...
          class="text-white absolute w-full hover:underline cursor-pointer top-0 font-bosemiboldld opacity-0 sm:opacity-100 group transition-all duration-300 ease-in-out ml-1"
        >
          {{ playlist.name }}
        </div>

        <div
          class="text-gray-300 text-[13px] flex opacity-0 sm:opacity-100 group transition-all duration-300 ease-in-out"
        >
          <div class="ml-1"></div>
          <div class="block">
          ...
          >
            <div class="ml-2 flex">
              ...
              <span v-if="playlist"
                >{{ playlist?.songs.length || 0 }} Canciones</span
              >
            </div>
          </div>
        </div>
        ...
        <div
          class="absolute flex gap-4 items-center justify-start bottom-0 mb-1.5 ml-1"
        >
          <button
          ...
          <button
            v-if="playlist.user_id === mainStore.user?.id"
            type="button"
            @click="showConfirmationModal = true"
            id="deletePlaylist"
          >
            <svg
              class="w-7 h-7 text-white hover:text-red-500 transition-all duration-300 ease-in-out"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 18 20"
