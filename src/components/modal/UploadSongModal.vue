              ...
              <div class="flex w-full mb-4">
                ...
                <div class="w-1/2 p-4">
                  <DialogTitle
                    ...
                    >¡Sube una canción!</DialogTitle
                  >
                  <div class="mt-4">
                    <label for="name" class="block text-sm text-gray-400"
                    ...
                    />
                    <p v-if="errors.name" class="text-xs text-red-600 mt-2">
                      {{ errors.name[0] }}
                    </p>
                  </div>

                  <div class="mt-4">
                    <label for="artist" class="block text-sm text-gray-400"
                    ...
                    <input
                      ...
                      class="w-[90%] h-10 my-2 py-3 px-4 truncate block border-6 bg-gray-950 text-white border-gray-200 rounded-md text-sm focus:border-green-500 focus:ring-green-500 shadow-sm"
                    />
                    <p v-if="errors.artist" class="text-xs text-red-600 mt-2">
                      {{ errors.artist[0] }}
                    </p>
                  </div>
                  ...
                  <div class="mt-4">
                    <label
                      ...
                      >Archivo de canción</label
                    >
                    <label
                      class="bg-[#121212] w-[90%] pt-[9px] pl-2 block h-10 text-sm text-white border border-gray-200 rounded-lg cursor-pointer relative overflow-hidden"
                    >
                      <input
                        @change="updateSongName"
                ...
                <div class="w-1/2 p-4">
                  <DialogTitle
                    as="h3"
                    class="text-white text-lg text-center font-medium leading-6 mb-4 sm:ml-6"
                    >Vista previa</DialogTitle
                  >
                  <div
                    class="flex items-center justify-center w-full pt-16 sm:pt-7"
                  >
                    <img
                      :src="coverArtPreview || '/images/cover-art-default.png'"
                      alt="Cover Art Preview"
                      class="w-36 h-36 sm:w-56 sm:h-56 sm:ml-8 rounded group transition-all duration-300 ease-in-out"
                    />
                  </div>
                  <div class="block pr-3 sm:pl-7">
                    <label
                      class="text-white text-sm block pt-7 text-center truncate"
                      >{{ formData.name || "Nombre" }}</label
                    >
                    <label
                      class="text-gray-500 font-semibold text-sm block text-center truncate"
                      >{{ formData.artist || "Artista" }}</label
                    >
                  </div>
                </div>
              </div>
...
const songName = ref("");
const coverArtName = ref("");
const coverArtPreview = ref(null);
...
const updateCoverArt = (event) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      coverArtPreview.value = e.target?.result;
    };
